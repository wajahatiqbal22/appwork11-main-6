...
class PBookingController extends GetxController {
  ...
  PBookingController() {
    _bookingRepository = BookingRepository(Get.find<ProviderApiClient>());
    _eProviderRepository = EProviderRepository(Get.find<ProviderApiClient>());
    _paymentRepository = PaymentRepository(Get.find<ProviderApiClient>());
  }
  ...
  Future<void> getBooking() async {
    try {
      ...
      if (booking.value.status == Get.find<PHomeController>().getStatusByOrder(Get.find<GlobalService>().global.value.inProgress!) && timer == null) {
...
//            val!.duration += (1 / 60);
  ...
  Future<void> acceptBookingService() async {
    if (booking.value.status!.order == Get.find<GlobalService>().global.value.received) {
      final _status = Get.find<PHomeController>().getStatusByOrder(Get.find<GlobalService>().global.value.accepted!);
      await changeBookingStatus(_status);
    }
  }

  Future<void> onTheWayBookingService() async {
    final _status = Get.find<PHomeController>().getStatusByOrder(Get.find<GlobalService>().global.value.onTheWay!);
    await changeBookingStatus(_status);
  }

  Future<void> readyBookingService() async {
    final _status = Get.find<PHomeController>().getStatusByOrder(Get.find<GlobalService>().global.value.ready!);
    await changeBookingStatus(_status);
  }

  Future<void> confirmPaymentBookingService() async {
    try {
      final _status = Get.find<PHomeController>().getStatusByOrder(Get.find<GlobalService>().global.value.done!);
  ...
  Future<void> declineBookingService() async {
    try {
      if (booking.value.status!.order! < Get.find<GlobalService>().global.value.onTheWay!) {
        final _status = Get.find<PHomeController>().getStatusByOrder(Get.find<GlobalService>().global.value.failed!);
  ...
  Future<void> startChat() async {
    ...
    Get.toNamed(Routes.PCHAT, arguments: _message);
  }
}
