...
class PEServiceFormController extends GetxController {
  ...
  void createEServiceForm({bool createOptions = false}) async {
    ...
    if (eServiceForm.currentState!.validate()) {
      try {
        ...
        print(_eService);
        if (createOptions)
          Get.offAndToNamed(Routes.POPTIONS_FORM, arguments: {'eService': _eService});
        else
          Get.offAndToNamed(Routes.PE_SERVICE, arguments: {'eService': _eService, 'heroTag': 'e_service_create_form'});
      } catch (e) {
        Get.showSnackbar(Ui.ErrorSnackBar(message: e.toString()));
      } finally {}
    } else {
  ...
  void updateEServiceForm() async {
    ...
    if (eServiceForm.currentState!.validate()) {
      try {
        ...
        Get.offAndToNamed(Routes.PE_SERVICE, arguments: {'eService': _eService, 'heroTag': 'e_service_update_form'});
      } catch (e) {
        Get.showSnackbar(Ui.ErrorSnackBar(message: e.toString()));
      } finally {}
    } else {
