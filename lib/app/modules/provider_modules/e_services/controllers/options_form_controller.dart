...
class POptionsFormController extends GetxController {
  final eService = EService().obs;
  final option = Option(checked: false.obs).obs;
  ...
  void _initOption({Map<String, dynamic>? arguments}) {
    if (arguments != null) {
      if (arguments['option'] != null) {
        option.value = (arguments['option'] as Option);
        print("option.value:${option.value}");
        print("option.value2:${arguments['option']}");
      }
    }
     else {
      option.value = new Option(checked: false.obs);
    }
    print(eService.value.id);
  ...
  * */
    ...
    if (optionForm.currentState!.validate()) {
      try {
        ...
        if (addOther) {
          ...
          await Get.offAndToNamed(Routes.PE_SERVICE, arguments: {'eService': eService.value, 'heroTag': 'option_create_form'});
        }
      } catch (e) {
        Get.showSnackbar(Ui.ErrorSnackBar(message: e.toString()));
      } finally {}
  ...
  void updateOptionForm() async {
    ...
    if (optionForm.currentState!.validate()) {
      try {
        ...
        Get.offAndToNamed(Routes.PE_SERVICE, arguments: {'eService': eService.value, 'heroTag': 'option_update_form'});
      } catch (e) {
        Get.showSnackbar(Ui.ErrorSnackBar(message: e.toString()));
      } finally {}
    } else {
  ...
  void deleteOption(Option option) async {
    try {
      ...
      Get.offAndToNamed(Routes.PE_SERVICE, arguments: {'eService': eService.value, 'heroTag': 'option_remove_form'});
    } catch (e) {
      Get.showSnackbar(Ui.ErrorSnackBar(message: e.toString()));
    }
  }
