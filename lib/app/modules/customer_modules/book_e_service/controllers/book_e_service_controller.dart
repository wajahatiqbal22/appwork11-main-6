...
class BookEServiceController extends GetxController {
  ...
  BookEServiceController() {
    _bookingRepository = BookingRepository(Get.find<CustomerApiClient>());
    _settingRepository = SettingRepository(Get.find<CustomerApiClient>());
  }
  ...
  void makePayment() async {
    try {
      ...
      print(booking.value);
      bool result =
          await Get.toNamed(Routes.CHECKOUT, arguments: booking.value) as bool;
      result ;
      print(result);
    } catch (e) {
  ...
  void createBooking() async {
    try {
      ...
      final newBooking = await _bookingRepository.add(booking.value);
      ...
      Get.offAndToNamed(Routes.CONFIRMATION, arguments: newBooking);
    } catch (e) {
  ...
  Future<void> showTimeSlotPicker() async {
   var result = await Get.toNamed(Routes.TIMESLOTS);
  }
