...
class SettingsService extends GetxService {
  ...
  SettingsService() {
    _settingsRepo = new SettingRepository(Get.find<CustomerApiClient>());
    _box = new GetStorage();
    if (isConsumer())
      _settingsRepo = new SettingRepository(Get.find<CustomerApiClient>());
    else
      _settingsRepo = new SettingRepository(Get.find<ProviderApiClient>());
  }
    ...
    try {
      if (_box.hasData('current_address')) {
        ...
        if (_addresses.isNotEmpty) {
          ...
          address.value = new Address(address: "Location".tr);
        }
      } else {
        address.value = new Address(address: "Location".tr);
      }
  ...
  bool isConsumer() {
    if (_box.hasData('user_type')) {
      return _box.read<bool>('user_type')!;
    } else
      return true;
  }

  Future setUserType(bool isCustomer) {
    return _box.write('user_type', isCustomer);
  }
}
